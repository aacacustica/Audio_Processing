...
def leq_levels_oct(audio_files:list ,fs_filterbanks:float , w_size: int, C:float, results_folder:str):
    ...
    for audio_file in tqdm(audio_files):
        try:        
            ...
            if len(db) > 0:
                ...
                df_all = pd.concat([df_history, df_all])
                
                #set filename and date as first columns
                df_all = df_all[['filename','date'] + [col for col in df_all.columns if col not in ['filename','date']]]
    ...
    # The last part changes to write JSON instead of CSV.
    file_ext = "_spl_oct.json"  # Change the file extension to .json
    ...
    file_name_json = os.path.join(results_folder, folder_name) 
    print(f"\nFile name json: {file_name_json}")  
    
    df_all.to_json(file_name_json, orient='index', date_format='iso', indent=4)
    
    logger.info(f'{len(audio_files)} Archivos procesados con éxito')
    logger.info(f'Archivo {file_name_json} creado con éxito')
