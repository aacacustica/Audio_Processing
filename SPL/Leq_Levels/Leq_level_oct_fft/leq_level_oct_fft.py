...
def leq_level_oct_fft(audio_files:list, fs_filterbanks:float , w_size:int , C:float, results_folder:str):
    """ Write a CSV with noise levels and 1/3 octave spectrum using fft algo of a list of audio files
    ...
    # filterbanks
    logger.info(f'Filtros de ponderaciones creados con fs {fs_filterbanks}')
    # compute edge frequencies
    f_lower, fm , f_upper = get_edge_frequencies(idx_low=-16,idx_high=12)
    assert f_upper[0] < fs_filterbanks ,"Wrong edge frequencies"
    
    col_names = ['LA','LC','LZ','LAmax','LAmin']
    band_names = [str(np.round(band,2)) for band in fm]
    ...
    for audio_file in tqdm(audio_files):      
        try:
            ...
            for fstart in range(0, len(x) - w_size + 1, w_size):
                ...
                oct_level_temp = third_octave_dft(frame, f_lower, f_upper, fs,C)
            ...
            if len(db) > 0:
                ...
                try:
                    ...
                    logger.error(f"Date parsing error for file {audio_file}")
                    start = datetime.datetime.now()  # or use a default date if parsing fails
    ...
    if "192.168.205.117" in results_folder:
        folder_name = results_folder.replace(" ", "_").split("\\")[6] + "_spl_oct_fft.csv"
...
if __name__ == '__main__':
    ...
    calibration_dict = {"24F3190361CBDDE4": -13.91,
                        "249BC30461CBDE81": -13.31,
                        "24E144025F5777C9": -13.5,
                        "24F3190160370890": -14.29,
                        "243B1F0663FA5A51": -13.43, # TENERIFE(FREDOLSEN) 
    ...
    if "artist" in list(metadata["tags"].keys()):
        ...
        logger.info("Calibration constant set to {C} for {device_id}")  
    else:
        C = -10.16
        logger.info("Calibration constant set to {C} for songmeter")

    w_size = int(fs_filterbanks) 
    
    leq_level_oct_fft(audio_files=valid_audio_files,
